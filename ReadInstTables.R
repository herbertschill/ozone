...
ReadIniFile <- function (nini, CoreName, isPth)
#############################################################################
#                                                                           #
# Reads the path-file 'CoreName.pth' with the adress of the ini-file        #
# 'CoreName.ini' (isPth=1), or opens the ini-file 'CoreName.ini' from the   #
# same directory as the calling program (isPth=0); reads the initialization #
# parameters from the ini-file 'CoreName.ini'                               #
#                                                                           #
#############################################################################
{

	# Create 'CoreName.pth' filename and test on existence, if isPth=1

	if (isPth==1)
	{
		aepath = "C:\\PMOD\\Programs\\Path\\"
		filename  = sprintf("%s%s", CoreName, ".pth")
		fpathname = sprintf("%s%s", aepath, filename)
		ex=file.exists(fpathname)

		if (ex)  # path file 'CoreName.pth' exists
		{
			pthfile = file(fpathname,open="r")

			# Open 'CoreName.pth', read line and extract pathname for "CoreName.ini"

			hline = ""
			hline = scan(pthfile,what="character",nlines=1,quiet=TRUE)
			iniPath=hline[1]
			close(pthfile)

			# Create 'CoreName.ini' filename, test on existence and open it

			filename  = sprintf("%s%s", CoreName, ".ini")
			fpathname = sprintf("%s%s", iniPath, filename)
		} else  # path file 'CoreName.pth' doesen't exist
		{
			infoFile = sprintf("%s%s%s","\n  Path-file  ",filename, "  not found !\n")
			cat (infoFile, file="")
			fpathname="0"
			iniPar=0
	...
	if (ex)
	{
		...
		for (i in 1:nini)
		{
			...
			if  (i==1)
				iniPar[i]=line[3]  # version from headerline
			else
				iniPar[i]=line[1]
		}
		iniOk=1
