...
#                                                                           #
#          - Allow treatment of multiple instruments with different         #
#            calibrations by reading an instrument string such as           #
#            e.g. '040,072,156', and a calibration string as e.g.           #
#            'Cal21a,Cal21d,Cal20f'; each instrument is proceeded in a      #
#            separate loop.                                                 #
#                                                                           #
#############################################################################
...
#	CalStr      = iniParams[[2]][[4]]
	...
	multInstStr0=iniParams[[2]][[3]]
	multInstStr=strsplit(iniParams[[2]][[3]],",")
	multCalStr=strsplit(iniParams[[2]][[4]],",")
	...
	for (inn in 1:nnInstr)
	{
		...
		{
			CalStr = multCalStr[[1]][[1]]
		}

		# screen-info about instrument loop

		infoFile = sprintf("%s%s%s%s%s%s%s","\n\n  Loop ",inn,"  Instrument to process: ",BrewerStr," (",CalStr,")\n\n")
		cat(infoFile, file="")
		...
		{
			# IF LocStr='VAR', read the values for all stations from the station list file,
			# else read the values for the desired station only
				...
				for (mon in mon1:mon2)
				{
					...
					for (day in day1:day2)
					{
						...
						if (InputType=="DS")
						{
							...
							{
								##########################################################################
								# Check on location (read from DS-file): 
								# if it has changed, set proper new station parameters
	...
	#						jdn;day;mon;year;brewStr;bPath
							...
							{
								##########################################################################
								# Check on location (read from B-file): 
								# if it has changed, set proper new station parameters
	...
	#
			...
			}  # for year=yearA:yearE (time loop)

		}  # end if iniOk

	}  # for inn=0:nnInstr-1 (instrument loop)

	infoFile = sprintf("%s%s%s%s%s%s%s","\n\n  Period ", PeriodA, "-", PeriodE,
